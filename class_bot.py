...
async def meme(ctx):
    ...
    with open(f'memes/Sybau.jpeg', 'rb') as f:
...
async def polusi(ctx):
    x = ['Kurangi penggunaan kendaraan bermotor untuk menjaga kualitas udara.','Menanam pohon bisa menyerap karbon dioksida dan menghasilkan oksigen.','Jangan membakar sampah karena menghasilkan zat beracun,']
    y = random.choice(x)
    await ctx.send(f":{y}")
bot.run
